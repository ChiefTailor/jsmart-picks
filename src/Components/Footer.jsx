...
const Footer = () => {
  ...
  return (
    <div className="bg-black h-[20vh] pt-5 flex items-center justify-between  px-10">
      <div>
        <h1 className="text-white text-2xl font-bold font-sans pl-4">
          JERRY SMART PICKS
        </h1>

        <nav className=" flex  items-center justify-between  z-20">
          <ul className="flex gap-8 p-4 text-l font-sans pt-10">
            {links.map((link, index) => (
              <li key={index} className="">
                <button
                  onClick={() => handleScroll(link.link)}
                  className="text-white hover:text-[#bf0050]"
                >
                  {link.name}
                </button>
              </li>
      ...
      <div className="pr-10"> 
        <div>
          <ul className="towel">
          <Link to= "https://discord.gg/hnUux68kAp">
          <li className="icon ">
              <svg
                fill="#000"
                width="600px"
        ...
        <p className="text-white pb-5">© 2025 | All rights reserved.</p>
      </div>
    </div>
  );
};
